...
function AddNewSessionDialog() {
  const [note, setNote] = useState<string>("");
  const [loading, setLoading] = useState(false);
  const [suggestedDoctor, setSuggestedDoctor] = useState<doctorAgent[]>([]); // ✅ changed from null to []
  ...
  const onClickNext = async () => {
    ...
    try {
      ...
      console.log("API response:", result.data); // ✅ log response for debugging

      // ✅ Handle both array or object with `suggestedDoctor` key
      const doctors = Array.isArray(result.data)
        ? result.data
        : result.data.suggestedDoctor;
